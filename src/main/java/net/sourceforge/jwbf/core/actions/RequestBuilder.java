...
public class RequestBuilder {
	...
	private final ImmutableMap.Builder<String, Supplier<String>> params = ImmutableMap
			.builder();
	private final ImmutableMap.Builder<String, Supplier<Object>> postParams = ImmutableMap
			.builder();
	...
	public RequestBuilder postParam(String key, double value, Locale locale,
			String format) {
	...
	public RequestBuilder param(String key, String value) {
		if (!Strings.isNullOrEmpty(key)) {
			if (Strings.isNullOrEmpty(value)) {
				...
				log.warn("Empty string for GET param \"" + key
						+ "\" was transformed to \"None\"");
			} else {
	...
	static class HashCodeEqualsMemoizingSupplier<T> implements Supplier<T>,
			Serializable {
